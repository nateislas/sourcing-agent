...
async def migrate():
    ...
    async with engine.begin() as conn:
        ...
        if "verification_status" not in columns:
            ...
            await conn.execute(text("ALTER TABLE entities ADD COLUMN verification_status VARCHAR DEFAULT 'UNVERIFIED'"))
        
        if "rejection_reason" not in columns:
            print("Adding 'rejection_reason' column...")
            await conn.execute(text("ALTER TABLE entities ADD COLUMN rejection_reason VARCHAR"))
            
        if "confidence_score" not in columns:
            print("Adding 'confidence_score' column...")
            await conn.execute(text("ALTER TABLE entities ADD COLUMN confidence_score INTEGER DEFAULT 0"))
