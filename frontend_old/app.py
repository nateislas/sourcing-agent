        ...
        with st.container(border=True):
            with st.form(key="search_form", border=False):
                topic = st.text_input(
                    ...
                    placeholder="e.g., CDK12 small molecule, preclinical, TNBC, China",
                )

                # Using form_submit_button handles "Enter" key correctly
                submitted = st.form_submit_button(
                    "Initialize Deep Scan", type="primary", use_container_width=True
                )
                ...
                if submitted:
                    if not topic:
                        ...
                        with st.status(
                            "Launching Orbital Agents...", expanded=True
                        ) as status:
                            try:
                                # Get the session ID from the workflow
                                session_id = asyncio.run(start_research(topic))
                                status.update(
                                    label="Scanning Initialized!", state="complete"
                                )
...
def monitor_page(session_id: str):
    ...
    # Main Content Area
    tab_dash, tab_strat, tab_explore = st.tabs(
        ["📊 Dashboard", "🎯 Research Strategy", "🌐 Link Explorer"]
    )
